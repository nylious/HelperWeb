...
export default async function SectionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (slug === 'items') {
    ...
    return (
      <>
        ...
        <SectionNav active="items" sections={(await import('@/lib/data')).getSections().then((items) => items.map((item) => item.slug))} />
        <ItemGenerators />
      </>
    )
  }
  ...
  return (
    <>
      ...
      <SectionNav active={slug} sections={(await import('@/lib/data')).getSections().then((items) => items.map((item) => item.slug))} />
